...
describe("Anthropic to OpenAI translation logic", () => {
  ...
  test("should preserve Claude Code model names", () => {
    const sonnetPayload: AnthropicMessagesPayload = {
      model: "claude-sonnet-4.6",
      messages: [{ role: "user", content: "Hello!" }],
      max_tokens: 100,
    }

    const opusPayload: AnthropicMessagesPayload = {
      model: "claude-opus-4.1",
      messages: [{ role: "user", content: "Hello!" }],
      max_tokens: 100,
    }

    expect(translateToOpenAI(sonnetPayload).model).toBe("claude-sonnet-4.6")
    expect(translateToOpenAI(opusPayload).model).toBe("claude-opus-4.1")
  })

  test("should translate nested custom tools from Claude Code", () => {
    const anthropicPayload: AnthropicMessagesPayload = {
      model: "claude-sonnet-4.6",
      messages: [{ role: "user", content: "Hello!" }],
      max_tokens: 100,
    ...
    const openAIPayload = translateToOpenAI(anthropicPayload)
    expect(openAIPayload.tools?.[0]?.function.name).toBe("get_weather")
