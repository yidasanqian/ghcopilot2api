...
export function getAnthropicToolName(tool: AnthropicTool): string | undefined {
  ...
  const customName =
    typeof customTool?.name === "string" ? customTool.name.trim() : ""
  if (customName) {
    return customName
  }
  ...
  const fallbackType = typeof tool.type === "string" ? tool.type.trim() : ""
...
export function getAnthropicToolDescription(
  tool: AnthropicTool,
): string | undefined {
  ...
  const customTool = getCustomToolDefinition(tool)
  if (typeof customTool?.description === "string") {
    return customTool.description
  }
  ...
  if (
    ...
    return `MCP tool ${tool.server_name}.${tool.server_tool_name}`
  }
...
export function getAnthropicToolInputSchema(
  ...
  const customTool = getCustomToolDefinition(tool)
  const customSchema = normalizeToolSchema(customTool?.input_schema)
  if (customSchema) {
    return customSchema
  }
